---
title: 'Final Project'
author: "STAT 420, Summer 2020, Sara Kohtz"
date: ''
output:
  html_document: 
    theme: readable
...
```
Based on the result, we found colinearity exist between: temp_max & Amb_Temp, t_38_35v & time_max_temp, Re7 & Rct7.

To do partial relation with the variables mentioned above and the response variable Capa
```{r}
if_tm1 = lm(Capa ~. - temp_max, data = cycle_data_full)
if_tm2 = lm(temp_max ~. - Capa, data = cycle_data_full)
cor(resid(if_tm1), resid(if_tm2))
```
Partial correlation temp_max and Capa is really small, therefore adding temp_max to the function may not be helpful. 
...
```
The partial correlation between Rct7 and Capa is 0.037, which is really small, therefore Rct7 may not have significant influcent over the Capa. 

Then, we try to model the data. Since `Amb_Temp` as categorical variable that includes 3 levels: 5, 20, 40, the `as.factor` is utilized. 
```{r}
cycle_data_full$Amb_Temp = as.factor(cycle_data_full$Amb_Temp)
```

Start with full two-way interaction model, and select model using backwards procedure based on BIC. 
...
Model diagnostics are performed on this first model `mod_selected`. First, some functions are created to easily test models, and determine "good" models with respect to: leave-one-out-cross-validation root mean square error (LOOCV-RMSE), adjusted r-squared values, and number of parameters. 
...
#SP test 
test_sp = function(model, alpha) {
  p_val = shapiro.test(resid(model))$p.value
  decision = ifelse(p_val < alpha,"Reject", "Fail to Reject")
  list1 = list(sp.p_val = p_val, sp.decision = decision)
  return(list1)
}
...
Based on the Log-likelihood, we will see $\lambda = 2$ is the best value. We create a new polynomial model below:
...
The Q-Q plot looks pretty good, and Shapiro-Wilk test fails to reject the null hypothesis, so it is safe to assume the normality assumption is not violated. 
```{r}
result_mod_selected_trans = run_all_diagnostic(mod_selected_trans)
```
```{r, echo=FALSE}
...
Although the fitted vs residuals plot suggests there are no issues with assuming equal variance, the model `mod_selected_trans` still fails the BP test.In order to address this issue, we want to revise the model by removing the non-significant predictor variables temp_max & Rct7. 
```{r}
mod_selected_trans_remove = lm((Capa^2-1)/2 ~ Amb_Temp + cycle + time_max_temp + 
    t_38_35V + Re7  + temp_max:Amb_Temp + temp_max:time_max_temp + 
    temp_max:t_38_35V + temp_max:Re7 + Amb_Temp:t_38_35V + cycle:time_max_temp + 
    cycle:Re7 + time_max_temp:Re7 + t_38_35V:Rct7, data = cycle_data_full, subset = keep)
```
...
```{r}
result_selected_trans_remove = run_all_diagnostic(mod_selected_trans_remove)
```

```{r, echo=FALSE}
kable(result_selected_trans_remove)
```


But the model doesn't pass either bp test or sharpilo test. 

We first try using variance inflation factors on our proposed model, the results are shown below:

```{r}
library(faraway)
vif(mod_selected_trans)
vif(mod_selected_trans_remove)
anova(mod_selected_trans, mod_selected_trans_remove)
```
Though the VIF for model mod_selected_trans is larger than mod_selected_trans_remove, it is better choice for the data based on the anova test. 
...
```{r}
#This model is randomly choose by excluding temp_max variable, which doesn't have significant influnce and we use log(capa) to try to improve the normality issue
...
In this model, it passes the bp test but failed the sharpilo test. Therefore, there's a trade off between the bptest and the sharpilo test. 
...
We have done some ablation study to prove that the proposed model is the best model when compared to the other models. Fully addictive model and full three-way interaction model are used for the ablation study.
...
```
And after utilizing the `summary` function, all of these coefficients are determined significant to the model, which makes sense because that was the purpose of utilizing BIC. The only was the electrolyte resistnace `Re7`, which makes sense because it would be very similar to transfer resistance `Rct7`, so both are not needed in the model. However, it is clear there are interactions and some non-linear relationship between capacity and other predictors. 
...
Also, we realized there's a trade off between the bp test and sharpilo test as we fitted many models but some of them can only satisfy one test. In reality, this is common for data proccessor to encounter and it should train our mindset on how should we balance the trade off and select a better model to improve the accuracy for data prediction 
