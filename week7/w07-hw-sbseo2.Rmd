...
Among simple model, addictive model, and interaction model, this interaction model shows superior result to the others. That is because slope and intercept is adjusted based on the type of vehicles.
...
Since $\alpha$ is greater than p-value, we reject null hypothesis. It means that interaction model is more preferred than addictive model.
...
summary(h_add)
```
Based on the alphabetical order(A-Z), the beginning words are chosen as reference level.
...
$$ Y = \beta_0 + 10\beta_1 + 139\beta_2 + \beta_3 + 1390\beta_4 + 10\beta_5 + 139\beta_6 + 1390\beta_7$$
...
$$ Y = \beta_0 + 10\beta_1 + 139\beta_2 + \beta_3 $$
