...
$H_0: \beta_1 =\beta_2 \ beta_3 = 0$ vs $H_1 :$ not $ $H_0$
...
- The null hypothesis
$$H_0: \beta_{Save Percentage} = \beta_{Goals Against Average} = \beta_{Penalties in Minutes} = 0$$
...
Since $p-value > \alpha$, do not reject $H_0$.

- The model you prefer

I prefer model2 to model3 because p-value is larger than alpha. 
...
- The value of the test statistic
```{r}
null = lm(W ~ GA      + SA + MIN + SO, data=goalies)
full = lm(W ~ GA + SV + SA + MIN + SO, data=goalies)
anova(null, full)$"F"[2]
```
