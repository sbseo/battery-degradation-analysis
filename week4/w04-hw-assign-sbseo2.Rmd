...
There are only `r ans` proportion whose beta_3_pval is less than 0.1. Considering my expected p-value is `r exp` where $\alpha = 0.1$, it is easy to expect that only few of the beta_3_pval would be less than 0.1. Therefore, this result matches with what I have expected.  
...
Considering my expected p-value is extremely small as in  `r exp`, it is easy to expect that values in beta_5_pval should be less than 0.01. Therefore, the result above is an expected outcome.
